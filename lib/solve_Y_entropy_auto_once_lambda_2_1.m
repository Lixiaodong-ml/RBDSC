function [Y, objHistory, res_aio] = solve_Y_entropy_auto_once_lambda_2_1(L, Y, lambda, e_type, gt)
%
% Method B: Discrete coordinate descent with GLOBAL ACCEPT RULE
...
%     No local approximations or delta tricks.
%     
%     current_obj = F(Y) / G(Y)
%     For each sample i, try all clusters p:
%         candidate_obj = F(Y_try) / G(Y_try)
%         if candidate_obj < current_obj - tol, ACCEPT
%         else, REJECT
%
