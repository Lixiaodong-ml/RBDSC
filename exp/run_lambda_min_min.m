...
% exp_n = 'max_cluster_add_noise_118_lambda_2_1_exp';
exp_n = 'min_min_exp';
...
%     load(data_name);
    ...
    % BMGC
    %*********************************************************************
    fname2 = fullfile(prefix_mdcs, [data_name(1:13), '_', exp_n, '.mat']);
...
%                 S0 = constructW_PKN_du(X', nSmp-2, 1);
...
%                 S0 = full(S0);
                ...
                for iEntropy = 1:length(entropy_range)
                    ...
                    fname3 = fullfile(prefix_mdcs, [data_name(1:13), '_12k_', exp_n, '_', num2str(iParam), '.mat']);
...
%                             label00(rIdx(1:ceil(nSmp*r_ratio))) = randi(nCluster, ceil(nSmp*r_ratio), 1);
                        ...
                        t2 = toc;
                        BSGC_PKN_NL_FINCH_auto_time(iParam) = t0 + t1 + t2/nRepeat;
                        save(fname3, 'result_11_s', 't0', 't2', 't1', 'knn_size', 'e_type','res_aio_Y','objHistory_Y','objHistory_whole','res_whole_aio','lambda_iter','label','o1_iter_obj','o2_iter_obj');
                    end
                end
             end
        end
        ...
        save(fname2, 'label_iter','BSGC_PKN_NL_FINCH_auto_result', 'BSGC_PKN_NL_FINCH_auto_grid_result', 'BSGC_PKN_NL_FINCH_auto_time', 'BSGC_PKN_NL_FINCH_auto_result_summary','result_Y_all_iter','obj_Y_all_iter','res_whole_iter','obj_whole_iter','lambda_whole_iter','o1_iter_obj_iter','o2_iter_obj_iter');
